...
const Services = () => {
  const services = [
    {
      icon: <Scissors className="h-10 w-10 text-white-600" />,
    ...
    {
      icon: <Palette className="h-10 w-10 text-white-600" />,
    ...
    {
      icon: <Package className="h-10 w-10 text-white-600" />,
    ...
    {
      icon: <Users style={{backgroundColor:"white"}} className="h-10 w-10 text-white-600 " />,
  ...
  return (
    <div className="py-16">
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-amber-50 to-white-100 py-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            {services.map((service, index) => (
              <div
                key={index}
                className="bg-stone-50 rounded-xl p-8 hover:shadow-lg transition-all duration-300 hover:bg-white-50"
              >
                <div className="flex items-center mb-6">
                  <div className="bg-white-100 rounded-full p-3 mr-4">
                    {service.icon}
                  </div>
                ...
                <ul className="space-y-2">
                  {service.features.map((feature, idx) => (
                    <li key={idx} className="flex items-center text-stone-600">
                      <div className="w-2 h-2 bg-white-600 rounded-full mr-3"></div>
                      {feature}
                    </li>
                  ))}
                </ul>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
            {process.map((item, index) => (
              <div key={index} className="text-center">
                <div className="bg-gradient-to-br from-white-600 to-amber-600 text-white w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold mx-auto mb-4">
                  {item.step}
                </div>
      ...
      {/* CTA Section */}
      <section style={{backgroundColor:"white"}} className="py-20 bg-gradient-to-br from-white-600 to-amber-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">
            Ready to Order Your Custom Chappals?
          </h2>
          <p className="text-xl text-white-100 mb-8 max-w-3xl mx-auto leading-relaxed">
            Experience the perfect blend of tradition and comfort. Let us create a pair that's uniquely yours.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href="/contact"
              className="bg-white text-white-600 px-8 py-3 rounded-lg font-semibold hover:bg-stone-100 transition-all duration-300 hover:shadow-lg"
            >
              Start Your Order
            </a>
            <a
              href="tel:+929112334567"
              className="border-2 border-white text-white px-8 py-3 rounded-lg font-semibold hover:bg-white hover:text-white-600 transition-all duration-300"
            >
              Call Us Now
            </a>
          </div>
